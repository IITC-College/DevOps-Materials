...
You need a cluster with **at least 2 worker nodes**. Everything runs in the `default` namespace.

```bash
kubectl get nodes
```

Write your node names down, you will type them a lot:

- Node A: `_______________`
- Node B: `_______________`

---

## Task 1 - Watch the scheduler do its job

1. Create the Pod in `01-scheduled-pod.yaml`.
...
1. Open `02-nodename-pod.yaml` and replace `CHANGE-ME` with **Node A**.
...
1. Look at `03-pending-pod.yaml` and find the field that makes this Pod different.
2. Create it, then check its status.
3. Describe the Pod. How many events does it have?
...
Now place `pending-pod` on **Node A** using the `Binding` object in `03-binding.yaml`:
...
2. Send the Binding to the API server.
3. Verify the Pod is now `Running` on that node, and describe it - **which node placed it?**

> **Hints**
>
> - Try `kubectl apply -f 03-binding.yaml` first. It fails - read the error.
> - A `Binding` is a write-only resource: the API only accepts `POST` on it, there is no `GET`. `apply` needs to read the object first, so it cannot work. Use `kubectl create -f 03-binding.yaml`.
...
`04-rgb-pending-pods.yaml` contains three unschedulable Pods: `red-pod`, `green-pod`, `blue-pod`.

1. Create all three and confirm all three are `Pending`.
2. Bind **red** to Node A, **green** to Node B, **blue** to Node A - one Binding at a time.
3. Verify the placement of all three in a single command.
4. Count how many lab Pods each node is running.

> **Hints**
>
> - Copy `03-binding.yaml` per Pod, or edit and re-`create` the same file three times.
...
> - `spec.nodeName` is immutable on a running Pod. The edit is refused.
...
- Why does `kubectl apply` fail on a Binding while `kubectl create` succeeds?
