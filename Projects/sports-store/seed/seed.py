...
def product(name, slug, description, category, gender, base_price, tags, variants):
    return {
        "name": name,
        "slug": slug,
...
def variant(sku, color, size, price, stock):
    return {"sku": sku, "color": color, "size": size,
...
def seed_products(client: MongoClient) -> int:
    products = client["catalog_db"]["products"]
    count = 0
    for doc in PRODUCTS:
        result = products.update_one(
            {"slug": doc["slug"]}, {"$setOnInsert": doc}, upsert=True
        )
...
def seed_admin(client: MongoClient) -> bool:
    users = client["auth_db"]["users"]
    if users.find_one({"email": ADMIN_USER["email"]}):
        return False
    users.insert_one(
        {
            **ADMIN_USER,
...
def main():
    ...
    created = seed_products(client)
    admin_created = seed_admin(client)
    total = client["catalog_db"]["products"].count_documents({})
    print(f"Seed complete: {created} new products inserted, {total} total.")
    print(f"Admin user {'created' if admin_created else 'already present'}: "
          f"{ADMIN_USER['email']}")
