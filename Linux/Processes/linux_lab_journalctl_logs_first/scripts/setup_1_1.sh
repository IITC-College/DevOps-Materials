#!/bin/bash
# BEFORE Level 1, Clue 1: Start lab-logger-ok service for journalctl practice.
set -e
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LAB_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
USER_UNIT_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/systemd/user"
UNIT_FILE="$USER_UNIT_DIR/lab-logger-ok.service"
TRACKING="$LAB_ROOT/.lab_1_1_service"

[ -f "$TRACKING" ] && { echo "Clue 1-1 service already running."; exit 0; }
...
echo "Clue 1-1: lab-logger-ok service started. Run journalctl (or journalctl --user -u lab-logger-ok)."
echo "Done."
